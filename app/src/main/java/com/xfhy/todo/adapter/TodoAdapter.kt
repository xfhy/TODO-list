 ...
 * Description : todo列表adapter
 */
class TodoAdapter(itemData: MutableList<TodoBean.Data.TodoItem>?, private val isDone: Boolean) :
        BaseSectionQuickAdapter<TodoBean.Data.TodoItem, BaseViewHolder>(R.layout.item_todo, R.layout.item_header_todo, itemData) {
    ...
    override fun convert(holder: BaseViewHolder, item: TodoBean.Data.TodoItem?) {
        //是否已完成
        if (isDone) {
            holder.setImageResource(R.id.iv_item_undo, R.mipmap.icon_done_todo)
        }
